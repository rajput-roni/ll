...
"""
IMPORTANT:
- Yeh script interactive inputs lene ke baad apne aap daemonize (background mein detach) ho jaati hai,
  jisse Termux exit hone ke baad bhi SMS bhejti rahe – chahe internet ya mobile off ho.
- GSM SMS fallback ke liye GSM module (jaise SIM800L/SIM900A) ka connection hona chahiye, aur iska 
  serial port (default: /dev/ttyUSB0) aur baudrate (115200) sahi se set hon.
- Unlimited token support: Token file mein har token ek alag line mein hona chahiye.
- Yeh script bina external command (nohup/tmux/screen vagairah) ke hi apne andar daemonize ho jaati hai,
  jisse sahi hardware support ke saath yeh 1 saal tak continuously chal sakti hai.
"""
...
# NEW: Agar multiple tokens hain, to har token ke liye alag thread se SMS bheje
